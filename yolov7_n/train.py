...
def train(hyp, opt, device, tb_writer=None):
    ...
    if rank in [-1, 0]:
        if is_h5_dataset:
            logger.info("Utilizzo del DataLoader custom per dataset .h5")
            testdataset = DatasetH5(path=test_path, img_size=imgsz)
            sampler = torch.utils.data.distributed.DistributedSampler(dataset) if rank != -1 else None
            ...
            # Logica originale di YOLOv7
            testloader, testdataset = create_dataloader(test_path, imgsz, batch_size, gs, opt,
                                                    hyp=hyp, augment=True, cache=opt.cache_images, rect=opt.rect, rank=rank,
                                                    world_size=opt.world_size, workers=opt.workers,
                                                    image_weights=opt.image_weights, quad=opt.quad, prefix=colorstr('train: '))
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            if is_h5_dataset:
            # Per i nostri dati H5, i valori sono già normalizzati [0,1], quindi non dividiamo.
                imgs = imgs.to(device, non_blocking=True).float()
            else:
                # Per le immagini normali, manteniamo la logica originale.
            ...
            # Forward
            with torch.amp.autocast(device_type='cuda', dtype=torch.float16, enabled=cuda):
            ...
            if rank in [-1, 0]:
                ...
                if plots and ni < 10:
                    ...
                    # Riga modificata in train.py
                    Thread(target=plot_images, args=(imgs, targets, paths, f), kwargs={'is_magnetogram': is_h5_dataset}, daemon=True).start()
                    # if tb_writer:
